...
Press **⌘⇧U** (Ctrl+Shift+U) where the quote should start, and **⌘⇧U** again where it should end: what was said in between is added as a quote row at your caret, starting with the start and end times as clickable timestamps (`12:40 - 13:05: ...`). While it runs, a red recording marker in the status bar shows how long it has been going (in video time, so it stands still while paused); click it to cancel. Nothing ends it on its own: after 5 minutes a reminder asks whether to keep going (another 5 minutes) or cancel. Two presses within a second also cancel. The words come from the video's own captions, including YouTube's automatic ones, fetched through [Supadata](https://supadata.ai). It needs your own Supadata API key (the plugin ships without one): create a free account at [supadata.ai](https://supadata.ai), copy your key, then run **"YouTube: Set transcript API key from clipboard"** from the command palette. The key is stored in your workspace's settings for this plugin, and until one is set, `Cmd+Shift+U` tells you how to add it. The transcript is fetched once per video (already at the first press, so the quote appears at once), and repeated quotes cost nothing extra. A video with no captions at all cannot be quoted.

![Transcribed quotes with their start and end times, and the recording marker in the status bar](screenshots/transcription.png)
